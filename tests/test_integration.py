"""End-to-end integration tests: straight road and corner-turn scenarios.

No other actors are present so tests focus purely on the routing →
path planning → MPC → bicycle-model pipeline.

To view the straight scenario in the Pygame visualiser run:
    uv run python main.py --rows 1 --cols 6

To view the corner scenario:
    uv run python main.py --rows 2 --cols 2
"""
...
def test_ego_reaches_destination(plot_telemetry) -> None:
    ...
    plot_telemetry(loop, "straight_reach_destination")
...
# ---------------------------------------------------------------------------
# Corner-turn scenario — 2×2 grid
#
# Node layout (node_id = row * grid_cols + col):
#   2 ── 3       NW ── NE
#   │    │   →   │     │
#   0 ── 1       SW ── SE
#
# Ego starts at node 0 (SW) heading east toward node 1 (SE).
# Destination is node 2 (NW), which requires a 90° left turn at node 1
# — or the router may choose 0 → 2 directly (north).
# Either way the heading must change by ≥ π/4 rad during the run.
# ---------------------------------------------------------------------------


def _make_corner_world() -> tuple[SimulationWorld, int]:
    """Create a 2×2 grid with ego at SW, destination at NE."""
    cfg = MapConfig(grid_rows=2, grid_cols=2, block_size_m=60.0)
    road_map, graph = generate_road_map(cfg, seed=0)

    # node 0 = SW, node 3 = NE
    start_node = road_map.nodes[0]
    dest_id = road_map.nodes[3].node_id
    node_by_id = {n.node_id: n.position for n in road_map.nodes}
...
def test_corner_heading_changes() -> None:
    """Ego must change heading by at least π/4 rad when navigating a corner."""
    world, dest_id = _make_corner_world()
    initial_heading = world.ego_state.pose.heading
    ...
    for _ in range(2000):
        loop.step()
        delta = abs(loop.world.ego_state.pose.heading - initial_heading)
        # Normalise to [0, π]
        delta = min(delta, 2 * math.pi - delta)
        if delta > max_heading_delta:
            max_heading_delta = delta

    assert max_heading_delta >= math.pi / 4, (
        f"Ego heading changed by only {math.degrees(max_heading_delta):.1f}°; "
        f"expected ≥ 45° turn"
    )


def test_corner_ego_reaches_destination(plot_telemetry) -> None:
    """Ego must come within 20 m of the NE corner within 2000 steps (~100 s)."""
    world, dest_id = _make_corner_world()
    node_by_id = {n.node_id: n.position for n in world.road_map.nodes}
    dest_pos = node_by_id[dest_id]
    ...
    plot_telemetry(loop, "corner_reach_destination")

    assert reached, (
        f"Ego did not reach NE corner within 2000 steps. "
        f"Final position: ({loop.world.ego_state.pose.x:.1f}, "
        f"{loop.world.ego_state.pose.y:.1f}), "
        f"destination: ({dest_pos.x:.1f}, {dest_pos.y:.1f})"
    )
