...
_WAYPOINT_PASS_DIST = 8.0  # metres — skip waypoints within this radius (already passed)
...
def _waypoints_ahead(ego: Pose, positions: list[Vector2]) -> list[Vector2]:
    ...
    Prefers waypoints that are both ahead (positive heading dot product) and
    at least _WAYPOINT_PASS_DIST away — this avoids re-targeting nodes the
    ego has already effectively passed (e.g. an intermediate corner node).
    Falls back to the nearest any-direction waypoint when nothing else qualifies.
    """
    cos_h = math.cos(ego.heading)
    sin_h = math.sin(ego.heading)

    best_distant_idx: int | None = None
    best_distant_dist = math.inf
    best_any_idx = 0
    ...
    for i, pos in enumerate(positions):
        ...
        if forward > 0 and dist > _WAYPOINT_PASS_DIST and dist_sq < best_distant_dist:
            best_distant_dist = dist_sq
            best_distant_idx = i

    idx = best_distant_idx if best_distant_idx is not None else best_any_idx
